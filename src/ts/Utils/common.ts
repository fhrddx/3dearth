...
//获取某个点的附近随机的点
export const getNearRandomPoint = (option) => {
  const {x, y, z, r} = option;
  const newX = x + (Math.random() -0.5) * 2 * r;
  const newY = y + (Math.random() -0.5) * 2 * r;
  const newZ = z + (Math.random() -0.5) * 2 * r;
